...
## 项目简介
一个 **Godot 4.6 EditorPlugin**（位于 `addons/godot_mcp/`），在 Godot 内部原生实现了 MCP（Model Context Protocol）服务器，无需 Node.js 依赖。提供 **179 个工具**（30 核心 + 149 补充），分为 6 大类，供 AI 助手读取和修改项目。
...
│   └── project_tools_native.gd # 34 个工具 — 项目信息/设置、资源、输入映射、自动加载、全局类、测试运行器、诊断、反向资源依赖、迁移检查、弃用 API 扫描、GDExtension 检测、渐变/可绘制纹理、PCK 打包、渲染输出
├── ui/
