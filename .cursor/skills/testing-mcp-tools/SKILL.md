...
- **补充工具默认禁用**：`tools/list` 只返回已启用工具（核心 30 个）。补充工具要在面板 **Tool Manager** 标签里勾选启用。计数显示在表头：`Core: 30/30 | Supp: N/163 | Total: M/193`。
...
2. **前置态证据**：`tools/list`（补充工具不在列）+ 禁用态 `tools/call`（返回 `Tool is disabled`）。截 Tool Manager 表头（`Total: x/193`）。
...
- 工具数必须恰好是 `Total: x/193`——少一个说明分类器/注册漏了。
