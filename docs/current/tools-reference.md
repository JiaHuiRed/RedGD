...
Godot MCP Native 实现了 **179 个工具**，分为 6 大类（含核心和补充工具）：
...
### 17. set_control_offset_transform

设置 Control 节点的 **Godot 4.7 偏移变换**（`offset_transform_position/rotation/scale/pivot`，以及 `enabled`、`visual_only`）。偏移变换可在不影响布局的前提下移动/旋转/缩放控件。仅修改传入的字段。低于 Godot 4.7 时返回 `status="unsupported"`。

**参数**：
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `node_path` | string | 是 | Control 节点路径（如 `/root/MainScene/UI/Panel`） |
| `enabled` | boolean | 否 | 切换 `offset_transform_enabled` |
| `position` | object | 否 | 偏移位置 `{x, y}`（像素） |
| `rotation` | number | 否 | 偏移旋转（弧度） |
| `scale` | object | 否 | 偏移缩放 `{x, y}` |
| `pivot` | object | 否 | 偏移变换轴心 `{x, y}`（像素） |
| `visual_only` | boolean | 否 | 为 `true` 时偏移仅为视觉效果，不影响输入/鼠标拾取 |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `status` | string | `"success"` 或 `"unsupported"` |
| `node_path` | string | 节点路径 |
| `applied` | Array[string] | 实际应用的属性名列表 |
...
**行为**：
- 节点不是 Control 时返回错误
- Godot 4.7 之前不存在 `offset_transform_*` 属性，返回 `status="unsupported"`
- 成功修改后标记场景为未保存

---

### 18. set_collision_one_way

为 2D 碰撞节点（`CollisionShape2D` 或 `CollisionPolygon2D`）启用/禁用**单向碰撞**，并可选设置 margin 与方向。`CollisionShape2D` 的单向碰撞及 `one_way_collision_direction` 需要 Godot 4.7；属性缺失时返回 `status="unsupported"`。

**参数**：
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `node_path` | string | 是 | CollisionShape2D 或 CollisionPolygon2D 节点路径 |
| `enabled` | boolean | 否 | 是否启用单向碰撞（默认 `true`） |
| `margin` | number | 否 | 可选 `one_way_collision_margin`（像素） |
| `direction` | object | 否 | 可选 `one_way_collision_direction`，`{x, y}`（Godot 4.7+） |
...
**行为**：
- 节点不是 CollisionShape2D/CollisionPolygon2D 时返回错误
- `CollisionPolygon2D.one_way_collision` 在 4.6+ 可用；`CollisionShape2D` 的单向碰撞与方向需要 4.7
- 不支持的属性会被跳过，若无任何属性可应用则返回 `status="unsupported"`
- 成功修改后标记场景为未保存
...
### 21. batch_read_scripts

在一次调用中读取多个脚本文件的内容。支持 `.gd` 和 `.cs` 文件。每个请求的路径返回一个结果项，减少读取多个脚本时的往返次数。

**参数**：
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `script_paths` | array<string> | 是 | 要读取的脚本路径数组（如 `["res://scripts/player.gd", "res://scripts/enemy.gd"]`） |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `status` | string | `"success"` |
| `count` | int | 结果项数量（等于请求的路径数） |
| `error_count` | int | 读取失败的条目数 |
| `results` | array | 每个路径一个条目；成功项含 `script_path`/`content`/`line_count`，失败项含 `script_path`/`error` |
...
校验 Godot 着色器（`.gdshader` 文件或内联 Shader 代码），无需 GPU。报告着色器能否解析，以及 `shader_type`、`render_modes`、`uniforms` 和带行号的结构性问题（缺失/非法 shader_type，花括号/圆括号/方括号不配平）。Godot 4.6+ 通用。

**参数**：
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `shader_path` | string | 否 | 要校验的着色器文件路径（如 `res://shaders/water.gdshader`），与 `content` 二选一 |
| `content` | string | 否 | 直接校验的着色器源码（与 `shader_path` 二选一） |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `valid` | boolean | 着色器是否解析通过 |
| `shader_type` | string | 解析出的着色器类型（如 `canvas_item`、`spatial`） |
| `render_modes` | Array[string] | 解析出的渲染模式列表 |
| `uniforms` | Array[Dictionary] | uniform 列表（仅在 valid 时填充） |
| `issues` | Array[Dictionary] | 结构性问题列表 |
| `issue_count` | int | 问题数量 |
| `godot_version` | string | 当前 Godot 版本 |

**uniform 条目结构**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `name` | string | uniform 名称 |
| `type` | int | 变量类型（Variant.Type 枚举值） |
| `hint_string` | string | 提示字符串 |

**问题条目结构**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `line` | int | 行号（0 表示无具体行） |
| `severity` | string | 严重级别（`error`） |
| `message` | string | 问题描述 |

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`

**行为**：
- 在 `shader_type` 行后注入哨兵 uniform，再通过 `Shader.get_shader_uniform_list()` 判断是否解析通过——4.6/4.7 行为一致
- `shader_path` 和 `content` 至少提供一个
- 引擎详细的着色器编译诊断（含精确行号）会写入 Godot 输出日志，无法通过脚本 API 取回
...
### 55. add_debugger_capture_prefix
...
### 66. update_runtime_node_property
...
### 68. evaluate_runtime_expression
...
### 76. batch_update_node_properties
...
### 78. batch_get_node_properties

在一次调用中读取多个节点的属性。每个请求的节点路径返回一个结果项，减少检查多个节点时的往返次数。

**参数**：
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `node_paths` | array<string> | 是 | 要读取的节点路径数组（如 `["/root/MainScene/Player", "/root/MainScene/Enemy"]`） |
...
| `count` | int | 结果项数量 |
| `error_count` | int | 读取失败的条目数 |
| `results` | array | 每个节点一个条目；失败项含 `node_path`/`error` |

**注解**：`readOnlyHint=true`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

---

### 79. batch_connect_signals

在一次调用中连接多个节点信号。每个请求的连接返回一个结果项，减少连接多个信号时的往返次数。

**参数**：
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `connections` | array<object> | 是 | 信号连接数组，每项含 `emitter_path`/`signal_name`/`receiver_path`/`receiver_method`（必需）和 `flags`（可选） |
...
| `status` | string | `"success"` |
| `count` | int | 结果项数量 |
| `error_count` | int | 连接失败的条目数 |
| `results` | array | 每个连接一个条目（连接成功的结果，或含 `error` 的失败结果） |

**注解**：`readOnlyHint=false`, `destructiveHint=false`, `idempotentHint=false`, `openWorldHint=false`

---

### 80. audit_scene_node_persistence
...
### 82. list_project_script_symbols
...
### 83. find_script_symbol_definition
...
### 84. find_script_symbol_references
...
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `action` | string | 否 | `status`（默认）、`install`、`remove` |
| `tpz_path` | string | 否 | `install` 时：导出模板 `.tpz`/`.zip` 的路径 |
| `version` | string | 否 | `remove` 时：要删除的已安装版本目录名（如 `4.7.0.stable`） |
| `templates_root` | string | 否 | 覆盖模板目录；默认使用当前平台的编辑器模板目录 |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `action` | string | 执行的操作 |
| `templates_root` | string | 使用的模板目录 |
| `current_version` | string | 当前引擎版本（`status`） |
| `download_url` | string | 官方模板下载 URL（`status`） |
| `tpz_filename` | string | 对应 `.tpz` 文件名（`status`） |
| `installed_versions` | array | 已安装版本目录列表（`status`） |
| `installed_version` | string | 安装/删除涉及的版本（`install`/`remove`） |
| `extracted_count` | int | 解压的文件数（`install`） |
| `removed_count` | int | 删除的文件数（`remove`） |

**注解**：`readOnlyHint=false`, `destructiveHint=true`, `idempotentHint=false`, `openWorldHint=false`

---

### 97. configure_android_export
...
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `preset` | string | 是 | 预设名称或节名（如 `Android` 或 `preset.0`） |
| `config_path` | string | 否 | `export_presets.cfg` 路径（默认 `res://export_presets.cfg`） |
| `package_name` | string | 否 | 反向 DNS 应用 ID → `package/unique_name` |
| `app_name` | string | 否 | 显示名 → `package/name` |
| `version_code` | int | 否 | → `version/code` |
| `version_name` | string | 否 | → `version/name` |
| `use_gradle_build` | boolean | 否 | → `gradle_build/use_gradle_build` |
| `export_format` | string | 否 | `apk`/`aab` → `gradle_build/export_format` |
| `min_sdk` / `target_sdk` | string | 否 | → `gradle_build/min_sdk` / `gradle_build/target_sdk` |
| `architectures` | array | 否 | `arm64-v8a`/`armeabi-v7a`/`x86`/`x86_64` 子集；列出的启用，其余禁用 |
| `keystore_release` / `keystore_debug` | string | 否 | keystore 文件路径（仅路径，无密码） |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `status` | string | `success` |
| `config_path` | string | 写入的配置路径 |
| `preset` | object | 预设节/名称/平台 |
| `changes` | array | 应用的 `{key, value}` 改动列表 |
| `change_count` | int | 改动数量 |
...
|------|------|------|
| `status` | string | `success` |
| `unsaved_scenes` | array | 有未保存改动的场景路径 |
| `unsaved_scripts` | array | 有未保存改动的脚本路径 |
| `unsaved_scene_count` | int | 未保存场景数 |
| `unsaved_script_count` | int | 未保存脚本数 |
| `has_unsaved_changes` | boolean | 是否存在任意未保存改动 |
| `scenes_supported` | boolean | 当前 Godot 是否支持场景未保存查询（4.7+） |
| `scripts_supported` | boolean | 当前 Godot 是否支持脚本未保存查询（4.7+） |
...
| `status` | string | `success` 或 `unsupported` |
| `message` | string | 说明信息 |
| `godot_version` | string | 当前引擎版本（仅 `unsupported` 时） |

**注解**：`readOnlyHint=false`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

---

### 100. reload_open_scripts
...
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `script_path` | string | 否 | 要关闭的脚本路径（如 `res://scripts/player.gd`），默认当前聚焦脚本 |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `status` | string | `success` 或 `unsupported` |
| `closed_script` | string | 被关闭的脚本路径 |
| `message` | string | 说明信息 |
| `godot_version` | string | 当前引擎版本（仅 `unsupported` 时） |
...
|------|------|------|
| `status` | string | `success` |
| `scanning` | boolean | 文件系统是否正在扫描 |
| `scanning_progress` | number | 扫描进度（0–1） |
| `importing` | boolean/null | 是否正在导入（不支持时为 `null`） |
| `importing_supported` | boolean | 当前 Godot 是否支持导入状态查询（4.7+） |
| `busy` | boolean | 是否正在扫描或导入 |
...
### 119. get_runtime_performance_snapshot
...
### 123. simulate_runtime_input_event
...
### 124. simulate_runtime_input_action
...
### 125. list_runtime_input_actions
...
### 126. upsert_runtime_input_action
...
### 127. remove_runtime_input_action
...
### 131. get_runtime_animation_state
...
### 132. get_runtime_animation_tree_state
...
### 133. set_runtime_animation_tree_active
...
### 134. travel_runtime_animation_tree
...
### 135. get_runtime_material_state
...
### 137. set_runtime_theme_override
...
### 138. clear_runtime_theme_override
...
### 139. get_runtime_shader_parameters
...
### 140. set_runtime_shader_parameter
...
### 141. list_runtime_tilemap_layers
...
### 149. run_project_test

运行单个项目测试脚本，**不阻塞编辑器**。Python 集成测试使用 python 执行，GUT 单元测试通过 Godot headless 执行。

测试在后台线程中运行：**首次调用**启动测试并返回 `status="pending"`；用**相同的 `test_path` 再次调用**即可轮询，直到返回最终结果（`passed` / `failed`）。这与运行时探针的"先 pending、后轮询"模式一致，避免长时间运行的子进程冻结编辑器。

**参数**：
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `test_path` | string | 是 | `res://` 下的测试文件路径 |
| `timeout_ms` | int | 否 | 调用方的超时提示（保留字段） |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `status` | string | 运行中为 `"pending"`，完成后为 `"passed"` 或 `"failed"` |
| `framework` | string | 测试框架：`python` 或 `gut` |
| `test_path` | string | 测试文件路径 |
| `exit_code` | int | 退出码（完成后） |
| `elapsed_ms` | int | `pending` 时已运行的时间（毫秒） |
| `command` | array | 执行的命令（完成后） |
| `output` | array | 输出内容（完成后） |

**注解**：`readOnlyHint=false`, `destructiveHint=false`, `idempotentHint=false`, `openWorldHint=false`

---

### 150. run_project_tests
...
### 151. list_project_input_actions
...
### 152. upsert_project_input_action
...
### 153. remove_project_input_action
...
### 155. list_project_global_classes
...
### 157. inspect_csharp_project_support
...
### 158. compare_render_screenshots
...
### 165. scan_missing_resource_dependencies
...
### 166. scan_cyclic_resource_dependencies
...
### 171. scan_migration_compatibility
...
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `resource_path` | string | 是 | 保存路径（如 `res://textures/grad.tres`） |
| `fill` | string | 否 | 填充模式：`linear`、`radial`、`square`、`conic`（默认 `linear`） |
| `colors` | array | 否 | 渐变停靠点。每项为颜色字符串/数组，或 `{offset, color}`；省略时默认黑→白 |
| `fill_from` | object | 否 | 起点 `{x, y}`（0..1 比例，默认 `{x:0, y:0}`） |
| `fill_to` | object | 否 | 终点 `{x, y}`（0..1 比例，默认 `{x:1, y:0}`） |
...
|------|------|------|
| `status` | string | `"success"` 或 `"unsupported"` |
| `resource_path` | string | 已保存路径 |
| `fill` | string | 填充模式名称 |
| `fill_mode_value` | integer | 填充模式枚举值（linear=0, radial=1, square=2, conic=3） |
| `stop_count` | integer | 渐变停靠点数量 |
| `godot_version` | string | 运行的 Godot 版本 |

**注解**：`readOnlyHint=false`, `destructiveHint=false`, `idempotentHint=false`, `openWorldHint=false`

**行为**：
- 路径经 `PathValidator` 校验，仅允许 `.tres`/`.res` 扩展名
- 无效的填充模式名返回错误
- 请求 conic 而引擎低于 4.7 时返回 `status="unsupported"`
...
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `pck_path` | string | 是 | 输出归档路径（如 `res://packs/dlc.pck` 或 `user://dlc.pck`） |
| `files` | array | 是 | 要打包的文件。每项为源路径字符串（按相同 `res://` 路径打包）或 `{target_path, source_path}` |
| `alignment` | integer | 否 | 打包文件的字节对齐（默认 `32`） |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `status` | string | `"success"` |
| `pck_path` | string | 归档路径 |
| `packed_count` | integer | 成功打包的文件数 |
| `size_bytes` | integer | 归档文件大小（字节） |
| `skipped` | array | 被跳过的条目（含原因：缺少源路径、源未找到、add_file 失败） |

**注解**：`readOnlyHint=false`, `destructiveHint=false`, `idempotentHint=false`, `openWorldHint=false`

**行为**：
- 路径经 `PathValidator` 校验，仅允许 `.pck`/`.zip` 扩展名
- 源文件不存在的条目会被跳过并记入 `skipped`
- 没有任何文件被打包时返回错误

---

### 177. configure_render_output
...
| 参数 | 类型 | 必需 | 描述 |
|------|------|------|------|
| `hdr_2d` | boolean | 否 | 启用 HDR 2D 输出（Godot 4.7 `rendering/viewport/hdr_2d`） |
| `transparent_background` | boolean | 否 | 设置 `rendering/viewport/transparent_background` |
| `persist` | boolean | 否 | 通过 `ProjectSettings.save()` 持久化到 project.godot（默认 `true`） |

**返回值**：
| 字段 | 类型 | 描述 |
|------|------|------|
| `status` | string | `"success"` |
| `persisted` | boolean | 是否已持久化到磁盘 |
| `changes` | array | 每项设置的结果（`setting`、`status`=`updated`/`unsupported`、`previous`/`new`） |
| `godot_version` | string | 运行的 Godot 版本 |

**注解**：`readOnlyHint=false`, `destructiveHint=false`, `idempotentHint=true`, `openWorldHint=false`

**行为**：
- 设置键不存在（如旧版本无 `hdr_2d`）时该项报告 `status="unsupported"`，不会新建键
- 未提供任何受支持设置时返回错误
- `persist=false` 时仅在内存中修改，不写入 project.godot

---

### 178. create_drawable_texture
...
本手册详细说明了 Godot MCP Native 项目的所有核心工具及部分补充工具。项目共 **179 个工具**（30 核心 + 149 补充），所有工具均可通过 MCP 工具管理面板按分组动态启用/禁用。补充工具（`*-Advanced` 分组）默认不启用，需在工具管理面板中手动开启。
