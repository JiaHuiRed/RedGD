...
Godot-MCP 实现了 **185 个工具**，分为 6 大类和两种级别：

**核心工具（30 个）**：默认启用，始终可见。
**补充工具（155 个）**：需在工具管理面板中手动启用。
...
| Project Tools | 3 | 38 | 41 | 项目配置（信息、设置、资源、自定义/批量资源创建与属性读写、测试、输入映射、自动加载、全局类、诊断、反向资源依赖、迁移检查、弃用 API 扫描、GDExtension 检测、渐变纹理创建、PCK 打包、渲染输出配置、可绘制纹理创建与绘制） |
