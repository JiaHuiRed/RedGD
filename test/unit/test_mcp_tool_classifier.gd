...
func test_all_193_tools_registered():
	var all_tools: Array = _classifier.get_all_tools()
	assert_eq(all_tools.size(), 193, "Should have exactly 193 tools registered")
...
func test_supplementary_tools_count():
	...
	assert_eq(supp_tools.size(), 163, "Should have 163 supplementary tools")
